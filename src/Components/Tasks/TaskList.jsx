...
const TaskList = ({
  ...
  // Add keyboard shortcuts for task edit actions
  // useEffect(() => {
  //   const handleKeyPress = (e) => {
  //     if (e.altKey && (e.key === "e" || e.key === "E")) {
  //       e.preventDefault();
  //       // Open the task name edit for the first task (you can modify for specific task logic)
  //       onEdit(tasks[0]);
  //     }
  //     if (e.altKey && (e.key === "d" || e.key === "D")) {
  //       e.preventDefault();
  //       // Trigger work description edit for the first task
  //       setOpenWorkDescPopup(tasks[0]?._id);
  //       setWorkDescMode("edit");
  //     }

  //     if (e.altKey && (e.key === "r" || e.key === "R")) {
  //       e.preventDefault();
  //       // Trigger remark edit for the first task
  //       setOpenRemarkPopup(tasks[0]?._id);
  //       setRemarkMode("edit");
  //     }
  //   };
  ...
  // Rendering task card for small screen (mobile)
//  const renderTaskCard = (task, index) => (
//   <div
//     key={task._id}
//     className="relative border border-gray-900 rounded-2xl p-6 mb-6 shadow-md hover:shadow-2xl transition duration-300 overflow-hidden max-w-md mx-auto"
//     style={{ borderColor: colors.border }}
//     aria-label={`Task card number ${index + 1} for ${task.taskName}`}
//   >
//     {/* Serial Number Badge */}
//     {/* <div className="absolute top-4 left-4 bg-purple-300 text-white rounded-full w-7 h-7 flex items-center justify-center font-bold text-lg shadow-lg select-none">
//       {index + 1}
//     </div> */}

//     {/* Mobile: Copy button at top-right */}
//     <button
//       className="lg:hidden absolute top-4 right-4 p-2 rounded-md border border-gray-300 text-gray-500 hover:text-gray-900 hover:bg-gray-100 shadow-md transition-colors duration-200"
...
//     {/* Title */}
//     <h3
//       className="text-3xl font-extrabold  font-mono mb-5 tracking-tight leading-tight text-transparent  bg-clip-text bg-gradient-to-r from-gray-600 via-black to-gray-100 select-text "
...
//     {/* Description Section */}
//     <div className="mb-6">
//       <span className="block text-gray-900 font-semibold tracking-wide mb-3 select-none ">
...
//       >
//         <div >
//           <span className="text-[17px] text-gray-400 subpixel-antialiased break-words flex-1">
//             {(task.workDesc || "No description").length > 90
//               ? `${task.workDesc.slice(0, 90)}…`
//               : task.workDesc || "No description"}
//             {task.code && (
//               <span
//                 className="text-indigo-700 underline cursor-pointer ml-3 font-semibold hover:text-indigo-900"
...
//     {/* Details */}
//     <div
//       className="text-md mb-5 flex gap-5 sm:flex-row sm:space-x-6 space-y-2 sm:space-y-0 "
//       style={{ color: colors.textSecondary }}
//       aria-label="Assignment and due dates"
//     >
//       <div>
//         <div className="font-bold text-gray-700 subpixel-antialiased select-none">
//           Date of Work:
//         </div>
//         <time dateTime={task.assignedDate} className="select-text font-medium text-gray-500">
...
//           Due Date:
//         </div>
//         <time dateTime={task.dueDate} className="select-text font-medium text-gray-500">
...
//         <span
//           className="font-bold subpixel-antialiased text-sm select-none"
//           style={{ color: colors.textPrimary }}
//         >
//           Status:
//         </span>
...
//         <span
//           className="ml-1 px-3 py-1 rounded-full text-sm font-semibold cursor-pointer select-none transition-colors border shadow-sm"
//           style={{
//             backgroundColor:
...
//             <div
//               ref={dropdownRef}
//               className="absolute rounded-2xl shadow-2xl border border-gray-300 w-44 mt-2 z-50 bg-white"
//               style={{
//                 position: "absolute",
...
//                 backgroundColor: colors.surface,
//                 boxShadow:
//                   "0 15px 30px rgba(0,0,0,0.1), 0 6px 12px rgba(0,0,0,0.05)",
//                 borderRadius: "16px",
//               }}
//             >
...
//                   <div
//                     key={statusOption}
//                     className="px-6 py-3 text-sm cursor-pointer select-none hover:bg-purple-200 rounded-2xl transition-colors duration-200"
//                     style={{
//                       color:
...
//       <span
//         className="font-bold mr-3 text-sm select-none"
//         style={{ color: colors.textPrimary }}
//       >
//         Team:
//       </span>
...
//           <span
//             key={assignee.email}
//             className="text-sm py-1 px-2 rounded-full border select-none font-semibold shadow-sm transition-colors duration-300 hover:bg-purple-100 cursor-default"
//             title={assignee.name}
//             style={{
...
//     {/* Remark Section */}
    ...
    //   )}
    //   <button
    //     className="ml-3 text-indigo-500 hover:text-indigo-800 focus:outline-none transition-colors duration-150"
    //     onClick={() => {
    //       setOpenRemarkPopup(task._id);
    //       setRemarkMode("edit");
    //     }}
    ...
    //               }}
    //               className="px-6 py-2 text-sm rounded-2xl font-semibold text-white bg-indigo-700 hover:bg-indigo-800 transition"
    //               type="button"
    //             >
    //               Save
    //             </button>
...
//     {/* Action Buttons */}
//     <div className="mt-6 flex flex-wrap items-center justify-start gap-5 ml-1">
//       {/* Edit and Completed Buttons */}
//       <div className="grid grid-cols-2 gap-5 w-full sm:w-auto">
//         <button
//           onClick={() => onEdit(task)}
//           className="text-sm rounded-2xl font-semibold whitespace-nowrap focus:outline-none transition border border-blue-600 text-blue-900 bg-blue-100 hover:bg-blue-600 hover:text-white shadow-lg"
...
//           className="px-6 py-2 text-sm rounded-2xl font-semibold whitespace-nowrap focus:outline-none transition text-green-900 border border-green-400 bg-green-100 hover:bg-green-500 hover:text-white shadow-lg"
...
//       <button
//         className="hidden lg:inline-flex p-4 rounded-full focus:outline-none transition shadow-lg"
//         onClick={() => handleCopyTask?.(task)}
//         title="Copy Task"
//         aria-label={`Copy task ${task.taskName}`}
//         style={{
...
//         type="button"
//       >
//         <FontAwesomeIcon icon={faCopy} className="h-7 w-7" />
//       </button>
...
//           className="p-4 rounded-full focus:outline-none transition shadow-lg"
//           title="Delete Task"
  ...
  <>
    ...
    {openTaskPopup === task._id && (
      <div className="fixed inset-0  bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4  ">
        <div className="bg-gray-100  rounded-2xl shadow-3xl w-full ml-17  p-6 relative overflow-y-auto max-h-[90vh] border border-gray-400 "
        style={{ width: "78%", maxWidth: "inherit" }} // ✅ popup same width as card
         >
 ...
 const renderTaskRow = (task, index) => (
  ...
  >
    ...
    <td className="py-3 px-4 text-center cursor-pointer select-none">
      ...
      <td
        className="py-3 px-4"
        style={{ color: colors.textSecondary, minWidth: "170px" }}
      >
        <div className="flex flex-wrap gap-2">
          {task.assignees?.map((assignee) => (
            <span
        ...
        <table className="w-full table-auto border-collapse text-sm  shadow-gray-900  text-gray-800 hidden lg:table">
          ...
          >
            <tr className="uppercase tracking-wide text-[11px] font-extrabold text-gray-900 select-none ">
              ...
              <th className="py-2.5 px-4 text-left min-w-[130px]">Task Name</th>
              ...
              <th className="py-2.5 px-4 text-left min-w-[160px]">Remarks</th>
              <th className="py-2.5 px-4 text-left min-w-[130px]">Team</th>
              <th className="py-2.5 px-4 text-left min-w-[80px]">
                Assigned By
              </th>
              <th className="py-2.5 px-4 text-center min-w-[40px]">Copy</th>
              {role === "admin" && (
                <th className="py-2.5 px-4 text-center min-w-[40px]">Delete</th>
              )}
            </tr>
          </thead>
