...
const Sidebar = () => {
  ...
  useEffect(() => {
    const updateLeaveAlert = () => {
      ...
      console.log(
        "🔁 Detected localStorage change → showLeaveAlert:",
        leaveFlag
      );
  ...
  return (
    ...
    <div className="bg-[#1e1f21] text-white h-screen flex flex-col justify-between border-r border-gray-700 w-[70px] ">
      ...
      {/* Main Navigation */}
      <div className="flex-1 overflow-y-auto px-3 py-4 pt-2 overflow-x-hidden">
        {/* Core nav */}
        <SidebarItem icon={<FaHome className="text-xl"/>} label="Home" to="/" />
        {role === "admin" && (
          <>
            <SidebarItem
        ...
        <SidebarItem icon={<FaClipboardList className="text-xl"/>} label="Tasks" to="/all-tasks" />
        <SidebarItem icon={<FaBriefcase className="text-xl"/>} label="Clients" to="/clients" />
        <SidebarItem
          icon={
            <div className="relative">
              <FaInbox />
        ...
        <SidebarItem icon={<FaClock className="text-xl"/>} label="Reminders" to="/reminders" />
        ...
        {role === "admin" && (
          <SidebarItem icon={<FaCog className="text-xl"/>} label="Settings" to="/departments" />
        )}
        ...
        {role === "admin" && (
          <SidebarItem icon={<FaMoneyBill className="text-xl"/>} label="Invoice" to="/invoice" />
        )}
...
const SidebarItem = ({ icon, label, to, onClick }) => (
  ...
  >
    ...
    <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-0.5 px-2 py-1 bg-blue-500 text-white text-sm rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-[9999] pointer-events-none">
       <span className="whitespace-normal break-words max-w-[150px] overflow-hidden text-ellipsis">{label}</span>
      {/* Tooltip arrow */}
      <div className="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-b-0 border-t-4 border-l-transparent border-r-transparent border-t-blue-500"></div>
    </span>
  </NavLink>
);
