...
export default function ViewInvoices() {
  ...
  useEffect(() => {
    axios
      .get("https://taskbe.sharda.co.in/api/clients/details")
      .then((res) => {
  ...
  useEffect(() => {
    ...
    const fetchInvoicesByClient = async () => {
      try {
        console.log("Fetching invoices for client:", selectedClient);
        const res = await axios.get(
          `https://taskbe.sharda.co.in/api/invoices?clientId=${selectedClient.value}`
        );
        const filteredInvoices = res.data || [];
        const sortedInvoices = [...filteredInvoices].sort(
          (a, b) => new Date(b.invoiceDate) - new Date(a.invoiceDate)
  ...
  const handleDeleteInvoice = async (invoiceNumber) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(
          `https://taskbe.sharda.co.in/api/invoices/${invoiceNumber}`
        );
        setInvoices((prev) =>
