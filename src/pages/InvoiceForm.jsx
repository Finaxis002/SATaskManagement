...
//    const [hasFetchedClientsFromView, setHasFetchedClientsFromView] = useState(!!propClients);
//    const [isInitialized, setIsInitialized] = useState(false);
//   const firmCache = useRef(new Map());
...
//   //     return;
//   //   }
//   //   console.log("Selected firm ID changed:", selectedFirmId);
//   //   console.log("Firms changed:", firms.length);
//   //   const newSelectedFirm = firms.find((x) => x._id === selectedFirmId) || null;

//   //   // Compare with the current selectedFirm state
//   //   if (newSelectedFirm !== selectedFirm) {
//   //     setSelectedFirm(newSelectedFirm);
//   //   }

//   //   setSelectedBankIndex(0);
//   // }, [selectedFirmId, firms, selectedFirm]); // Add selectedFirm to dependencies

//   // useEffect(() => {
//   //   if (isEdit) return;

//   //   // Use a ref to track the last processed firm+type combination
//   //   const currentCombo = `${selectedFirm?._id}-${invoiceType}`;

//   //   if (selectedFirm && invoiceType && currentCombo !== lastComboRef.current) {
//   //     lastComboRef.current = currentCombo;
//   //     setInvoiceNumber("");
//   //     setIsFinalized(false);
//   //     previewInvoiceNumber();
//   //   }
//   // }, [isEdit, selectedFirm?._id, invoiceType]);
...
//   //       const response = await axios.get("${baseURL}/clients/details");
...
//   // // Effect 1: Reset when no client selected
//   // useEffect(() => {
...
//   // // Effect 2: Set customer data when client changes
//   // useEffect(() => {
...
//   //   });
//   // }, [selectedClientOption, clients]);

//   // // Effect 3: Fetch tasks only when needed (with debounce)
//   // useEffect(() => {
//   //   if (!selectedClientOption || !fromDate || !toDate) return;
...
//   // }, [selectedClientOption, fromDate, toDate, clients]);
...
// const lastFirmTypeCombo = useRef("");
// const hasFetchedClients = useRef(false);
// const prevFirmIdRef = useRef(null);

//   // Wrap previewInvoiceNumber with useCallback
// const previewInvoiceNumber = useCallback(async () => {
//   if (isEdit) return;
...
//   useEffect(() => {
//     const loadFirms = async () => {
//       if (firmCache.current.size > 0) {
//         const cachedFirms = Array.from(firmCache.current.values());
//         setFirms(cachedFirms);
//         if (!isEdit && cachedFirms.length) {
//           setSelectedFirmId(cachedFirms[0]._id);
//           setSelectedFirm(cachedFirms[0]);
//         }
//         setFirmsLoading(false);
//         return;
//       }
...
//         console.log("Firms loaded:", firmsData);
//         firmsData.forEach(firm => {
//           firmCache.current.set(firm._id, firm);
//         });

//         setFirms(firmsData);
//         if (!isEdit && firmsData.length) {
//           setSelectedFirmId(firmsData[0]._id);
...
//   // Set initialized when both firms and clients are loaded
//   useEffect(() => {
//     if ((propClients || hasFetchedClients) && !firmsLoading) {
//       setIsInitialized(true);
//     }
//   }, [propClients, hasFetchedClients, firmsLoading]);

//   // // Show loading state
//   // if (!isInitialized) {
//   //   return <div>Loading invoice form...</div>;
//   // }

// // 2. Fixed Firm Selection (NO circular dependency)
// useEffect(() => {
//   if (!selectedFirmId) {
//     setSelectedFirm(null);
//     prevFirmIdRef.current = null;
//     return;
//   }

//   // Only proceed if firm ID actually changed
//   if (selectedFirmId === prevFirmIdRef.current) return;

//   const newSelectedFirm = firms.find((x) => x._id === selectedFirmId) || null;
//   setSelectedFirm(newSelectedFirm);
//   setSelectedBankIndex(0);

//   prevFirmIdRef.current = selectedFirmId;
// }, [selectedFirmId, firms]); // REMOVED selectedFirm dependency

// // 3. Single Invoice Number Preview Effect
// useEffect(() => {
//   if (isEdit) return;

//   const currentCombo = `${selectedFirm?._id}-${invoiceType}`;

//   if (selectedFirm && invoiceType && currentCombo !== lastFirmTypeCombo.current) {
//     lastFirmTypeCombo.current = currentCombo;
//     setInvoiceNumber("");
//     setIsFinalized(false);
//     previewInvoiceNumber();
//   }
// }, [isEdit, selectedFirm?._id, invoiceType, previewInvoiceNumber]); // ADD previewInvoiceNumber

// // 4. Fixed Client Fetching (prevent double fetch)
...
//  useEffect(() => {
//     if (hasFetchedClientsFromView || propClients) return;

//     const fetchClients = async () => {
//       try {
//         const response = await axios.get('/clients/details');
//         setClients(response.data);
//         setHasFetchedClientsFromView(true);
//       } catch (error) {
//         console.error("Error fetching clients:", error);
//       }
//     };
...
//     fetchClients();
//   }, [hasFetchedClientsFromView, propClients]);

// // 5. Client Selection Effects (OK)
// useEffect(() => {
//   if (!selectedClientOption) {
//     setCustomer({
//       _id: "", name: "", address: "", GSTIN: "", mobile: "", emailId: ""
//     });
//     setItems([]);
//   }
// }, [selectedClientOption]);
...
// // 6. Tasks Fetching with Proper Dependencies
// useEffect(() => {
//   if (!selectedClientOption || !fromDate || !toDate) return;
...
//   const notesArr = typeof inv.notes === "string"
//     ? inv.notes.split("\n").filter(Boolean).map((t) => ({ id: uuidv4(), text: t }))
//     : (inv.notes || []).map((n) => ({
//         id: uuidv4(),
//         text: typeof n === "string" ? n : n?.text || "",
//       }));
...
//   const invBank = f?.bank;
//   const idx = (f?.banks || []).findIndex(
//     (b) => (b?._id && invBank?._id && b._id === invBank._id) ||
//            (b.accountNumber || b.account) === (invBank?.accountNumber || invBank?.account)
//   );
//   setSelectedBankIndex(idx >= 0 ? idx : 0);
// }, [isEdit, initialInvoice]);

// // REMOVE the duplicate invoice preview effect
